    ...
    <div class="d-flex justify-center" v-if="$store.state.auth.user">
        <div>
            <div class="account-edit-btn-container">
                <v-btn icon small color="primary" @click="edit()"><v-icon size="small">fas fa-pen</v-icon></v-btn>
            </div>
            <div class="py-4 d-flex justify-center">
                ...
                >
                    ...
                    <div class="account-edit-photo-btn-container" @click="editPhotoDialog = true">
                        <v-icon color="white">fas fa-pen</v-icon>
                    </div>
                </v-avatar>
            </div>
            <h2 class="text-center primary--text my-4">{{ $store.state.auth.user.first_name }}</h2>
            <v-divider></v-divider>
            <p class="mt-4">{{ $store.state.auth.user.email }}</p>
        </div>
                    ...
                    <v-row justify="center">
                        ...
                        <div class="edit-photo-croppa" v-else>
                            <croppa
                                ...
                                :initial-image="$store.state.auth.user.photo ? $store.state.auth.user.photo : ''"
                            ></croppa>
                        </div>
                    </v-row>
                </v-card-text>
...
export default {
    ...
    methods: {
        edit() {
            this.$store.dispatch("users/edit", { data: this.$store.state.auth.user });
